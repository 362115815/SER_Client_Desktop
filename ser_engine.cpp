...
cSER_Engine::cSER_Engine(QString work_dir)//work_dir目录必须是全路径，且目录下面有SER_Engine文件夹
{
    ...
    sample_rate=16000;
    time_len=10;
    point_num=sample_rate*time_len;
    dataSource=new cDataSource(16000);
...
int cSER_Engine::update(QAbstractSeries *series)//更新数据
{
        ...
        if(server->isListening())
           {
            ...
            {
                cout<<"fail to close server."<<endl;
                return -1;
            }
        }
...
int cSER_Engine::start_recorder()
{
   ...
   openSmile->start_record_withRTDataOut(get_startIndex(),"127.0.0.1",6666);//启动 opensmile recorder with real time data out
...
int cSER_Engine::handle_new_connection()//处理新的连接
{
    ...
    if(socket)
    {
        cout<<"releasing old connection..."<<endl;

        socket->disconnectFromHost();

        cout<<"old connection released."<<endl;
    }
...
int cSER_Engine::socket_read_data()//读取socket的数据
{
    ...
    dataSource->push(s.toFloat());

    qDebug()<<"receiving data for client: "<<s.toFloat()<<endl;
